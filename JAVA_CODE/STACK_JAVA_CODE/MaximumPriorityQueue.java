 ...
 * Thus, a max-priority queue returns the element with maximum key first whereas,
 *  a min-priority queue returns the element with the smallest key first.
 */

/*
 * Heaps are great for implementing a priority queue because of the largest and smallest element at the root of the tree
 * for a max-heap and a min-heap respectively. We use a max-heap for a max-priority queue and a min-heap for a min-priority queue.
 */
...
class MaximumPriorityQueue {
	...
	public static void printHeap(int A[]) {
		for (int i = 1; i <= heapSize; i++) {
			System.out.println("Printing Heap : "+A[i]);
		}
		System.out.println("********");
	}
